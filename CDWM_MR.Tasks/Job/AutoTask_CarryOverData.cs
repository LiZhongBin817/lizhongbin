...
namespace CDWM_MR.Tasks.Job
{
    ...
    public class AutoTask_CarryOverData : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            var taknumber =await _planinfoservices.Query(c => c.finishstatus.Equals(2), c => new mr_planinfo() { id = c.id},false);
            if (taknumber.Count <= 0) return;//没有找到对应要转的信息
            string tasknumber = taknumber[0].mplannumber;
            await _planinfoservices.ExecutePro("insert into mr_planinfo_history(id,mplannumber,mplanname,mplanyear,mplanmonth,planstarttime,planendtime,createtime,createpeople,remark) select id,mplannumber,mplanname,mplanyear,mplanmonth,planstarttime,planendtime,createtime,createpeople,remark from mr_planinfo b where b.finishstatus=2 mplannumber=@maxpalnnumber", new { maxpalnnumber = tasknumber });
            ...
            await _mr_datainfoservices.ExecutePro("insert into mr_datainfo_history(id,meternum,autoaccount,username,address,telephone,meterbookname,meterbooknumber,mrreadernumber,mrreadername,areano,areaname,regionno,regionname,taskperiodname,lastmonthdata,inputdata,usewaternum,readstatus,readDateTime,uploadtime,readtype,meterstatus,readcheckdata,recheckresult,remark,readerid) select taskid as ID as id,meternum,autoaccount,username,address,telephone,bookname as meterbookname,bookno as meterbooknumber,mrreadernumber,mrreadername,areano,areaname,regionno,regionname,taskperiodname,lastmonthdata,nowmonthdata as inputdata,usewaternum,readstatus,readDateTime,uploadtime,readtype,meterstatus,readcheckdata,recheckresult,remark,readerid from v_mr_datainfo b where b.taskperiodname=@taskperiodname", new { taskperiodname = tasknumber });
            ...
            await _rt_b_faultinfoservices.ExecutePro("insert into rt_b_faultinfo_history(id,readdataid,faulttype,faultnumber,faulttype,meternum,taskperiodname,autoaccount,faultcontent,reporttime,gisinfo,meterstatus,readerid,reportpeople,faultstatus) select id,readdataid,faulttype,faultnumber,faulttype,meternum,taskperiodname,autoaccount,faultcontent,reporttime,gisinfo,meterstatus,readerid,reportpeople,faultstatus from rt_b_faultinfo b where b.taskperiodname=@taskperiodname", new { taskperiodname = tasknumber });
            ...
            await _rt_b_recheckservices.ExecutePro("insert into rt_b_recheck_history(id,readdataid,meternum,userid,taskperiodname,recheckstatus,recheckdata,recheckresult,metercode,checksuccesstime,checkor,createtime,createpeople) select id,readdataid,meternum,userid,taskperiodname,recheckstatus,recheckdata,recheckresult,metercode,checksuccesstime,checkor,createtime,createpeople from rt_b_recheck b where b.taskperiodname=@taskperiodname", new { taskperiodname = tasknumber });
